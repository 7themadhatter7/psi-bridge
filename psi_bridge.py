...
class PSIBridgeNode:
    ...
    def _broadcast_loop(self):
        ...
        while self.running:
            try:
                ...
                if self.outbox:
                    state_data["msgs"] = list(self.outbox)
                    self.outbox.clear()
    ...
    def _handle_incoming_state(self, data: dict):
        # Extract chat messages from payload
        if "msgs" in data and data["msgs"]:
            for msg in data["msgs"]:
                self.inbox.append(msg)
            if len(self.inbox) > 500:
                self.inbox = self.inbox[-500:]
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                if self.path == "/health":
                    ...
                    self._j({"endpoints": ["GET /health", "GET /coupling", "GET /lock", "GET /state", "GET /messages", "POST /state", "POST /send"]})
            def do_OPTIONS(self):
            ...
            def do_POST(self):
                if self.path == "/send":
                    ...
                    try:
                        msg = json.loads(raw)
                        node.msg_seq += 1
                        msg["seq"] = node.msg_seq
                        msg["ts"] = time.time()
                        node.outbox.append(msg)
                        self._j({"queued": True, "seq": node.msg_seq})
                    except Exception as e:
